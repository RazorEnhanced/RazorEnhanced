...
namespace Ultima
{
    public sealed class Multis
    {
        ...
        public static List<object[]> LoadFromDesigner(string fileName)
        {
            ...
            using (var idxfs = new FileStream(idx, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            using (var binfs = new FileStream(bin, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
