...
namespace Ultima
{
    public sealed class MultiComponentList
    {
        ...
        {
            // Clear upper bits (0x4000 = housing preview flag)
            graphic &= ~0x4000;
            ...
            Stream stream = Multis.Load(graphic) is MultiComponentList mcl ? new MemoryStream() : null;
            ...
            // Fallback: try reloading from index
            Stream rawStream = Multis.GetComponents(graphic) == MultiComponentList.Empty
                ? null
                : new MemoryStream();
            ...
            int length = 0;
            stream = Art.IsUOAHS()
                ? Multis.Load(graphic) is MultiComponentList _ ? stream : null
                : Multis.Load(graphic) is MultiComponentList _ ? stream : null;
            ...
            using (BinaryReader reader = new BinaryReader(stream))
            {
                ...
                int count = Art.IsUOAHS() ? length / 16 : length / 12;
                Count = count;
                SortedTiles = new MultiTileEntry[count];
                ...
                for (int i = 0; i < count; ++i)
                {
                    ...
                    SortedTiles[i].OffsetZ = reader.ReadInt16();
                    SortedTiles[i].Flags = reader.ReadInt32();
                    SortedTiles[i].Unk1 = Art.IsUOAHS() ? reader.ReadInt32() : 0;

                    var e = SortedTiles[i];

                    if (e.OffsetX < _min.X) _min.X = e.OffsetX;
                    if (e.OffsetY < _min.Y) _min.Y = e.OffsetY;
                    if (e.OffsetX > _max.X) _max.X = e.OffsetX;
                    if (e.OffsetY > _max.Y) _max.Y = e.OffsetY;
