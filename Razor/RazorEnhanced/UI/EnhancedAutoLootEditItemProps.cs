...
namespace RazorEnhanced.UI
{
	public partial class EnhancedAutolootEditItemProps : Form
	{
        ...
        public EnhancedAutolootEditItemProps(ListView PAutolootlistView, List<RazorEnhanced.AutoLoot.AutoLootItem> PAutoLootItemList, int PIndexSelected)
		{
			InitializeComponent();
            MaximizeBox = false;
			this.Text = m_Title;
        ...
        private void razorButton3_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < listViewProps.Items.Count; i++)
            {
                ...
                {
                    AutoLootItemList[IndexSelected].Properties.RemoveAt(y);
                    y--;
                }
                y++;
            }
        ...
        private void EnhancedAutolootEditItemProps_Load(object sender, EventArgs e)
        {
            ...
            lName.Text = AutoLootItemList[IndexSelected].Name;
            lGraphics.Text = "0x" + AutoLootItemList[IndexSelected].Graphics.ToString("X4");
            lColor.Text = "0x" + AutoLootItemList[IndexSelected].Color.ToString("X4");
