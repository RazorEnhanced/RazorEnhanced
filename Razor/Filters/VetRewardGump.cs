...
{
	internal class VetRewardGumpFilter : Filter
	{
		...
		internal override byte[] PacketIDs { get { return new byte[] { 0xB0, 0xDD }; } }

		internal override LocString Name { get { return LocString.VetRewardGump; } }
		...
		internal override void OnFilter(PacketReader p, PacketHandlerEventArgs args)
		{
			...
			if (Assistant.Client.Instance.ConnectionStart + TimeSpan.FromMinutes(1.0) < DateTime.Now)
				return;
