...
namespace Assistant.Macros
{
	internal class Macro
	{
		...
		internal int CurrentAction { get { return m_CurrentAction; } }
		...
		internal void DisplayTo(ListBox list)
		{
			m_ListBox = list;
		...
		internal void Insert(int idx, MacroAction a)
		{
			a.Parent = this;
		...
		internal bool Action(MacroAction action)
		{
			if (m_Recording)
			{
				action.Parent = this;
		...
		internal void Load()
		{
			...
			using (StreamReader reader = new StreamReader(m_Path))
			{
				...
				while ((line = reader.ReadLine()) != null)
				{
					...
					Type at = null;
					try { at = Type.GetType(args[0], false); }
					catch { }
					if (at == null)
						continue;
		...
		internal void Convert(MacroAction old, MacroAction newAct)
		{
			for (int i = 0; i < m_Actions.Count; i++)
			{
				if (m_Actions[i] == old)
