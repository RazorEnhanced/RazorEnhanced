...
namespace Assistant.Macros
{
	...
	internal class MacroMenuItem : MenuItem
	{
		...
		internal MacroMenuItem(LocString name, MacroMenuCallback call, params object[] args)
	...
	internal abstract class MacroAction
	{
		protected Macro m_Parent;

		internal MacroAction()
		{
		}
		...
		internal virtual MenuItem[] GetContextMenuItems()
		{
			return null;
		}

		internal Macro Parent { get { return m_Parent; } set { m_Parent = value; } }

		internal abstract bool Perform();
	}

	internal abstract class MacroWaitAction : MacroAction
	{
		protected TimeSpan m_Timeout = TimeSpan.FromMinutes(5);
		private DateTime m_Start;
		private MacroMenuItem m_MenuItem = null;

		internal MacroWaitAction()
		{
		}
		...
		internal MacroMenuItem EditTimeoutMenuItem
		{
			get
			{
				if (m_MenuItem == null)
	...
	internal class MacroComment : MacroAction
	{
		private string m_Comment;

		internal MacroComment(string comment)
		{
			if (comment == null)
				comment = "";
		...
		internal string Comment { get { return m_Comment; } set { m_Comment = value; } }
	...
	internal class DoubleClickAction : MacroAction
	{
		private Serial m_Serial;
		private ushort m_Gfx;

		internal DoubleClickAction(Serial obj, ushort gfx)
		{
			m_Serial = obj;
			m_Gfx = gfx;
		}

		internal DoubleClickAction(string[] args)
		{
	...
	internal class DoubleClickTypeAction : MacroAction
	{
		...
		internal DoubleClickTypeAction(ushort gfx, bool item)
		{
			m_Gfx = gfx;
			m_Item = item;
		}
	...
	internal class LiftAction : MacroWaitAction
	{
		...
		internal static Item LastLift { get { return m_LastLift; } set { m_LastLift = value; } }

		internal LiftAction(string[] args)
		{
			m_Serial = Serial.Parse(args[1]);
		...
		internal LiftAction(Serial ser, ushort amount, ushort gfx)
		{
			m_Serial = ser;
			m_Amount = amount;
			m_Gfx = gfx;
		...
		internal override bool PerformWait()
		{
	...
	internal class LiftTypeAction : MacroWaitAction
	{
		private ushort m_Gfx;
		private ushort m_Amount;

		internal LiftTypeAction(string[] args)
		{
			m_Gfx = Convert.ToUInt16(args[1]);
			m_Amount = Convert.ToUInt16(args[2]);
		}

		internal LiftTypeAction(ushort gfx, ushort amount)
		{
			m_Gfx = gfx;
			m_Amount = amount;
		}
	...
	internal class DropAction : MacroAction
	{
		private Serial m_To;
		private Point3D m_At;
		private Layer m_Layer;

		internal DropAction(string[] args)
		{
		...
		internal DropAction(Serial to, Point3D at)
			: this(to, at, 0)
		{
		}

		internal DropAction(Serial to, Point3D at, Layer layer)
		{
			m_To = to;
			m_At = at;
			m_Layer = layer;
	...
	internal class DropRelLocAction : MacroAction
	{
		private sbyte[] m_Loc;

		internal DropRelLocAction(string[] args)
		{
			m_Loc = new sbyte[3]
			{
		...
		internal DropRelLocAction(sbyte x, sbyte y, sbyte z)
		{
	...
	internal class GumpResponseAction : MacroAction
	{
		private int m_ButtonID;
		private int[] m_Switches;
		private GumpTextEntry[] m_TextEntries;

		internal GumpResponseAction(string[] args)
		{
		...
		internal GumpResponseAction(int button, int[] switches, GumpTextEntry[] entries)
		{
			m_ButtonID = button;
			m_Switches = switches;
	...
	internal class MenuResponseAction : MacroAction
	{
		private ushort m_Index, m_ItemID, m_Hue;

		internal MenuResponseAction(string[] args)
		{
		...
		internal MenuResponseAction(ushort idx, ushort iid, ushort hue)
		{
			m_Index = idx;
			m_ItemID = iid;
			m_Hue = hue;
	...
	internal class AbsoluteTargetAction : MacroAction
	{
		private TargetInfo m_Info;

		internal AbsoluteTargetAction(string[] args)
		{
		...
		internal AbsoluteTargetAction(TargetInfo info)
		{
			m_Info = new TargetInfo();
	...
	internal class TargetTypeAction : MacroAction
	{
		private bool m_Mobile;
		private ushort m_Gfx;

		internal TargetTypeAction(string[] args)
		{
			m_Mobile = Convert.ToBoolean(args[1]);
			m_Gfx = Convert.ToUInt16(args[2]);
		}

		internal TargetTypeAction(bool mobile, ushort gfx)
		{
			m_Mobile = mobile;
			m_Gfx = gfx;
		}
	...
	internal class TargetRelLocAction : MacroAction
	{
		private sbyte m_X, m_Y;

		internal TargetRelLocAction(string[] args)
		{
			m_X = Convert.ToSByte(args[1]);
			m_Y = Convert.ToSByte(args[2]);
		}

		internal TargetRelLocAction(sbyte x, sbyte y)
		{
			m_X = x;
			m_Y = y;
		}
	...
	internal class LastTargetAction : MacroAction
	{
		internal LastTargetAction()
		{
		}
	...
	internal class SetLastTargetAction : MacroWaitAction
	{
		internal SetLastTargetAction()
		{
		}
	...
	internal class SpeechAction : MacroAction
	{
	...
	internal class UseSkillAction : MacroAction
	{
		private int m_Skill;

		internal UseSkillAction(string[] args)
		{
			m_Skill = Convert.ToInt32(args[1]);
		}

		internal UseSkillAction(int sk)
		{
			m_Skill = sk;
		}
	...
	internal class ExtCastSpellAction : MacroAction
	{
		private Spell m_Spell;
		private Serial m_Book;

		internal ExtCastSpellAction(string[] args)
		{
			m_Spell = Spell.Get(Convert.ToInt32(args[1]));
			m_Book = Serial.Parse(args[2]);
		}

		internal ExtCastSpellAction(int s, Serial book)
		{
			m_Spell = Spell.Get(s);
			m_Book = book;
		}

		internal ExtCastSpellAction(Spell s, Serial book)
		{
			m_Spell = s;
			m_Book = book;
		}
	...
	internal class BookCastSpellAction : MacroAction
	{
		private Spell m_Spell;
		private Serial m_Book;

		internal BookCastSpellAction(string[] args)
		{
			m_Spell = Spell.Get(Convert.ToInt32(args[1]));
			m_Book = Serial.Parse(args[2]);
		}

		internal BookCastSpellAction(int s, Serial book)
		{
			m_Spell = Spell.Get(s);
			m_Book = book;
		}

		internal BookCastSpellAction(Spell s, Serial book)
		{
			m_Spell = s;
			m_Book = book;
		}
	...
	internal class MacroCastSpellAction : MacroAction
	{
		private Spell m_Spell;

		internal MacroCastSpellAction(string[] args)
		{
			m_Spell = Spell.Get(Convert.ToInt32(args[1]));
		}

		internal MacroCastSpellAction(int s)
		{
			m_Spell = Spell.Get(s);
		}

		internal MacroCastSpellAction(Spell s)
		{
			m_Spell = s;
		}
	...
	internal class SetAbilityAction : MacroAction
	{
		private AOSAbility m_Ability;

		internal SetAbilityAction(string[] args)
		{
			m_Ability = (AOSAbility)Convert.ToInt32(args[1]);
		}

		internal SetAbilityAction(AOSAbility a)
		{
			m_Ability = a;
		}
	...
	/*internal class DressAction : MacroWaitAction
	{
		...
		internal DressAction(string[] args)
		{
			m_Name = args[1];
		}

		internal DressAction(string name)
		{
			m_Name = name;
		}
	...
	/*internal class UnDressAction : MacroWaitAction
	{
		...
		internal UnDressAction(string[] args)
		{
			try
			{
		...
		internal UnDressAction(string name)
		{
			m_Name = name;
			m_Layer = 255;
		}

		internal UnDressAction(byte layer)
		{
			m_Layer = layer;
			m_Name = "";
		}
...
*/
	...
	internal class WalkAction : MacroWaitAction
	{
		...
		internal static DateTime LastWalkTime { get { return m_LastWalk; } set { m_LastWalk = value; } }

		internal WalkAction(string[] args)
		{
			m_Dir = (Direction)(Convert.ToByte(args[1])) & Direction.Mask;
		}

		internal WalkAction(Direction dir)
		{
		...
		internal static bool IsMacroWalk(byte seq)
		{
	...
	internal class WaitForMenuAction : MacroWaitAction
	{
		private uint m_MenuID;

		internal WaitForMenuAction(uint gid)
		{
			m_MenuID = gid;
		}

		internal WaitForMenuAction(string[] args)
		{
			if (args.Length > 1)
		...
		internal override bool CheckMatch(MacroAction a)
		{
	...
	internal class WaitForGumpAction : MacroWaitAction
	{
		private uint m_GumpID;
		private bool m_Strict;

		internal WaitForGumpAction()
		{
			m_GumpID = 0;
			m_Strict = false;
		}

		internal WaitForGumpAction(uint gid)
		{
			m_GumpID = gid;
			m_Strict = false;
		}

		internal WaitForGumpAction(string[] args)
		{
	...
	internal class WaitForTargetAction : MacroWaitAction
	{
		internal WaitForTargetAction()
		{
			m_Timeout = TimeSpan.FromSeconds(30.0);
		}

		internal WaitForTargetAction(string[] args)
		{
			try
			{
	...
	internal class PauseAction : MacroWaitAction
	{
		internal PauseAction(string[] args)
		{
			m_Timeout = TimeSpan.Parse(args[1]);
		}

		internal PauseAction(int ms)
		{
			m_Timeout = TimeSpan.FromMilliseconds(ms);
		}

		internal PauseAction(TimeSpan time)
		{
			m_Timeout = time;
		}
	...
	internal class WaitForStatAction : MacroWaitAction
	{
		private byte m_Direction;
		private int m_Value;
		private IfAction.IfVarType m_Stat;

		internal byte Op { get { return m_Direction; } }
		internal int Amount { get { return m_Value; } }
		internal IfAction.IfVarType Stat { get { return m_Stat; } }

		internal WaitForStatAction(string[] args)
		{
		...
		internal WaitForStatAction(IfAction.IfVarType stat, byte dir, int val)
		{
			m_Stat = stat;
			m_Direction = dir;
			m_Value = val;
	...
	internal class IfAction : MacroAction
	{
		internal enum IfVarType : int
		{
			Hits = 0,
			Mana,
			Stamina,
		...
		internal IfVarType Variable { get { return m_Var; } }
		internal string Counter { get { return m_Counter; } }

		internal IfAction(string[] args)
		{
		...
		internal IfAction(IfVarType var, sbyte dir, int val)
		{
			m_Var = var;
			m_Direction = dir;
			m_Value = val;
		...
		internal IfAction(IfVarType var, sbyte dir, int val, string counter)
		{
			m_Var = var;
			m_Direction = dir;
			m_Value = val;
		...
		internal IfAction(IfVarType var, string text)
		{
			m_Var = var;
	...
	internal class ElseAction : MacroAction
	{
		internal ElseAction()
		{
		}
	...
	internal class EndIfAction : MacroAction
	{
		internal EndIfAction()
		{
		}
	...
	/*internal class HotKeyAction : MacroAction
	{
		...
		internal HotKeyAction(KeyData hk)
		{
			m_Key = hk;
		}

		internal HotKeyAction(string[] args)
		{
			try
			{
	...
	internal class ForAction : MacroAction
	{
		...
		internal int Max { get { return m_Max; } }

		internal ForAction(string[] args)
		{
			m_Max = Convert.ToInt32(args[1]);
		}

		internal ForAction(int max)
		{
			m_Max = max;
		}
	...
	internal class EndForAction : MacroAction
	{
		internal EndForAction()
		{
		}
	...
	internal class ContextMenuAction : MacroAction
	{
		private ushort m_CtxName;
		private ushort m_Idx;
		private Serial m_Entity;

		internal ContextMenuAction(UOEntity ent, ushort idx, ushort ctxName)
		{
		...
		internal ContextMenuAction(string[] args)
		{
